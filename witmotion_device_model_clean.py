...
class DeviceModel:
    ...
    def __init__(self, deviceName, mac, callback_method):
        ...
        # Sliding window for health check
        from collections import deque
        self.sliding_window = deque(maxlen=50)
        self.sliding_config = {
            'enabled': False,
            'window_size': 50,
            'threshold': 1.5,
    ...
    def processData(self, Bytes):
        ...
        if self.sliding_config['enabled']:
    ...
    async def setOutputFreq(self, freq=50):
        ...
        # 写入频率
        await self.sendData(self.get_writeBytes(register, self.freqMap[closest_freq]))
        await asyncio.sleep(0.1)
    ...
    # Check sliding window health
    def check_sliding_window(self):
        """检查滑动窗口健康状态"""
        if not self.sliding_config['enabled'] or len(self.sliding_window) < 10:
            return {'healthy': True, 'percentage': 0.0}
        
        threshold = self.sliding_config['threshold']
        exceeded = sum(1 for mag in self.sliding_window if mag > threshold)
        percentage = (exceeded / len(self.sliding_window)) * 100
        healthy = percentage < self.sliding_config['trigger_percentage']
